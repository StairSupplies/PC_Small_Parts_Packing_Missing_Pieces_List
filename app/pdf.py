...
def createMissingLabel(orderNumber, resultsDF):
    #try:
        ...
        #qr = qrcode.make("{\"ProductID\": \""+ str(partNumber) +"\", \"Quantity\": "+ str(quantity) + "}")
        
        #save the QR code as a png
        #qr.save(os.getcwd() + "/app/static/qr_codes/qr_code_" + str(partNumber) + "_" + str(orderNumber) + ".png")
        ...
        #qr_path = os.getcwd() + "\\app\\static\\qr_codes\\qr_code_" + str(partNumber) + "_" + str(orderNumber) + ".png"
       ...
       # pdf.cell(1, .13, txt= PartDesc1, ln=True)
        ...
        pdf.cell(.01,.12, txt="ProductID         Finish        Missing", ln=True)
        #pdf.cell(.1,.12)
        ...
        for index, post in resultsDF.iterrows():
            ...
            pdf.cell(.1,.15, txt=str(resultsDF.at[index, "product_id"]) +"     "  + str(resultsDF.at[index, "lineItemFinishes"]) + "       " + str(resultsDF.at[index, "missing_quantity"]), ln=True)
